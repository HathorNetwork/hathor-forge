...
type PageType = "dashboard" | "explorer" | "wallet" | "nano-contracts" | "blocks" | "transactions" | "tokens" | "mining" | "logs" | "settings" | "api-explorer";
...
function App() {
  ...
  const { contracts, addContract, removeContract, updateContract } = useNanoContractStore();
  ...
  // Poll nano contract states
  useEffect(() => {
    if (nodeStatus !== "running" || contracts.length === 0) return;

    const fetchContractStates = async () => {
      for (const contract of contracts) {
        try {
          const state = await invoke<any>("get_nano_contract_state", { id: contract.id });
          if (state.success) {
            updateContract(contract.id, {
              blueprintId: state.blueprint_name || state.blueprint_id,
              balance: state.balance,
            });
          }
        } catch (e) {
          console.error(`Failed to fetch state for contract ${contract.id}:`, e);
        }
      }
    };

    fetchContractStates();
    const interval = setInterval(fetchContractStates, 5000);
    return () => clearInterval(interval);
  ...
  const handleSelectBlueprint = async (id: string) => {
    try {
      const info = await invoke<any>("get_blueprint_information", { id });
      if (info.success) {
  ...
  const handleInitializeContract = async () => {
    ...
    try {
      ...
      const response = await invoke<any>("headless_wallet_create_nano_contract", {
        request: {
  ...
  const handleOpenInteract = async (contract: any) => {
    try {
      const state = await invoke<any>("get_nano_contract_state", { id: contract.id });
      if (state.success) {
        const info = await invoke<any>("get_blueprint_information", { id: state.blueprint_id });
        if (info.success) {
  ...
  const handleCallMethod = async () => {
    ...
    try {
      ...
      const response = await invoke<any>("headless_wallet_call_nano_contract_method", {
        request: {
  ...
  const renderApiExplorer = () => (
    <div className="p-8 space-y-6">
      ...
      <div className="bg-[#0d1117] rounded-xl border border-slate-800 overflow-hidden flex-1">
        <SwaggerUIComponent specUrl={selectedApi === "fullnode" ? "/openapi-fullnode.json" : "/openapi-wallet.json"} />
      </div>
    </div>
  );
